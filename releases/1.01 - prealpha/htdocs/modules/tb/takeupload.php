...
loggedinorreturn();
    ...
    if (!in_array($ann, $GLOBALS['TBDEV']['announce_urls'], 1))
    ...
    $ret = $GLOBALS['xoopsDB']->queryF("INSERT INTO ".$GLOBALS['xoopsDB']->prefix("torrents")." (search_text, filename, owner, visible, info_hash, name, size, numfiles, type, descr, ori_descr, category, save_as, added, last_action, nfo, client_created_by) VALUES (" .
    ...
    @$GLOBALS['xoopsDB']->queryF("DELETE FROM ".$GLOBALS['xoopsDB']->prefix("files")." WHERE torrent = $id");
    ...
    $GLOBALS['xoopsDB']->queryF("INSERT INTO ".$GLOBALS['xoopsDB']->prefix("files")." (torrent, filename, size) VALUES ".file_list($filelist,$id));
   ...
   write_log(sprintf($GLOBALS['lang']['takeupload_log'], $id, $torrent, $GLOBALS['CURUSER']['username']));
    ...
    if (($fd1 = @fopen("rss.xml", "w")) && ($fd2 = fopen("rssdd.xml", "w")))
    {
      ...
      @fwrite($fd1, $s);
      @fwrite($fd2, $s);
      $r = $GLOBALS['xoopsDB']->queryF("SELECT id,name,descr,filename,category FROM ".$GLOBALS['xoopsDB']->prefix("torrents")." ORDER BY added DESC LIMIT 15") or sqlerr(__FILE__, __LINE__);
