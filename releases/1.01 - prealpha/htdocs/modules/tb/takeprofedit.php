...
loggedinorreturn();
    ...
    $GLOBALS['lang'] = array_merge( load_language('global'), load_language('takeprofedit') );
    
    if (!mkglobal("email:chpassword:passagain:chmailpass"))
      stderr("Update failed!", $GLOBALS['lang']['takeprofedit_no_data']);
    ...
    if ($chpassword != "") 
    {
      if (strlen($chpassword) > 40)
        stderr("Update failed!", $GLOBALS['lang']['takeprofedit_pass_long']);
      if ($chpassword != $passagain)
        stderr("Update failed!", $GLOBALS['lang']['takeprofedit_pass_not_match']);
      ...
      logincookie($GLOBALS['CURUSER']['id'], $passhash);
    }

    if ($email != $GLOBALS['CURUSER']["email"]) 
    {
      if (!validemail($email))
    ...
    $r = $GLOBALS['xoopsDB']->queryF("SELECT id FROM categories") or sqlerr();
    $rows = mysql_num_rows($r);
    for ($i = 0; $i < $rows; ++$i)
    {
      $a = $GLOBALS['xoopsDB']->fetchArray($r);
    ...
    $avatar = trim( urldecode( $_POST["avatar"] ) );
      ...
      if( !empty($avatar) ) 
      {
        ...
        if($img_size == FALSE || !in_array($img_size['mime'], $GLOBALS['TBDEV']['allowed_ext']))
          stderr($GLOBALS['lang']['takeprofedit_user_error'], $GLOBALS['lang']['takeprofedit_image_error']);

        if($img_size[0] < 5 || $img_size[1] < 5)
          stderr($GLOBALS['lang']['takeprofedit_user_error'], $GLOBALS['lang']['takeprofedit_small_image']);
    ...
    */
    ...
    if ($changedemail) {
      ...
      $body = str_replace(array('<#USERNAME#>', '<#SITENAME#>', '<#USEREMAIL#>', '<#IP_ADDRESS#>', '<#CHANGE_LINK#>'),
                        array($GLOBALS['CURUSER']['username'], $GLOBALS['TBDEV']['site_name'], $email, $_SERVER['REMOTE_ADDR'], "{$GLOBALS['TBDEV']['baseurl']}/confirmemail.php?uid={$GLOBALS['CURUSER']['id']}&key=$hash&email=$obemail"),
                        $GLOBALS['lang']['takeprofedit_email_body']);
      
      
      mail($email, "{$GLOBALS['TBDEV']['site_name']} {$GLOBALS['lang']['takeprofedit_confirm']}", $body, "From: {$GLOBALS['TBDEV']['site_email']}");
