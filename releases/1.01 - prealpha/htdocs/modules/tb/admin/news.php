...
require_once "include/html_functions.php";
    ...
    if('update' == $mode)
    {
      if(isset($input['news_update']) && count($input['news_update']))
      {
        ...
        {
          if(!is_valid_id($v)) stderr("Error", "No ".$GLOBALS['xoopsDB']->prefix("news")." ID");
          $newsIDS[] = $v;
        }
      }
      else
      ...
      @$GLOBALS['xoopsDB']->queryF("UPDATE ".$GLOBALS['xoopsDB']->prefix("news")." set added = ".time()." WHERE id IN ($news)");
    ...
    if ($mode == 'delete')
    {
      ...
      @$GLOBALS['xoopsDB']->queryF("DELETE FROM ".$GLOBALS['xoopsDB']->prefix("news")." WHERE id=$newsid") or sqlerr(__FILE__, __LINE__);
    ...
    if ($mode == 'add')
    {
      ...
      @$GLOBALS['xoopsDB']->queryF("INSERT INTO ".$GLOBALS['xoopsDB']->prefix("news")." (userid, added, body, headline) VALUES ({$GLOBALS['CURUSER']['id']}, $added, $body, $headline)") or sqlerr(__FILE__, __LINE__);
    ...
    if ($mode == 'edit')
    {
      ...
      $res = @$GLOBALS['xoopsDB']->queryF("SELECT * FROM ".$GLOBALS['xoopsDB']->prefix("news")." WHERE id=$newsid") or sqlerr(__FILE__, __LINE__);
      ...
      if ($_SERVER['REQUEST_METHOD'] == 'POST')
      {
        ...
        @$GLOBALS['xoopsDB']->queryF("UPDATE ".$GLOBALS['xoopsDB']->prefix("news")." SET body=$body, headline=$headline WHERE id=$newsid") or sqlerr(__FILE__, __LINE__);
      ...
      else
      {
        ...
        print  stdhead($GLOBALS['lang']['news_edit_title']) . $HTMLOUT . stdfoot();
        exit();
      }
    }
    ...
    $res = @$GLOBALS['xoopsDB']->queryF("SELECT * FROM ".$GLOBALS['xoopsDB']->prefix("news")." ORDER BY added DESC") or sqlerr(__FILE__, __LINE__);
    ...
    print stdhead($GLOBALS['lang']['news_window_title']) . $HTMLOUT . stdfoot();
    die;
?>