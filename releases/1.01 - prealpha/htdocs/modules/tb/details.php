...
$res = $GLOBALS['xoopsDB']->queryF("SELECT torrents.seeders, torrents.banned, torrents.leechers, torrents.info_hash, torrents.filename, LENGTH(torrents.nfo) AS nfosz, torrents.last_action AS lastseed, torrents.numratings, torrents.name, IF(torrents.numratings < {$GLOBALS['TBDEV']['minvotes']}, NULL, ROUND(torrents.ratingsum / torrents.numratings, 1)) AS rating, torrents.comments, torrents.owner, torrents.save_as, torrents.descr, torrents.visible, torrents.size, torrents.added, torrents.views, torrents.hits, torrents.times_completed, torrents.id, torrents.type, torrents.numfiles, categories.name AS cat_name, users.username FROM ".$GLOBALS['xoopsDB']->prefix("torrents")." LEFT JOIN ".$GLOBALS['xoopsDB']->prefix("categories")." ON torrents.category = categories.id LEFT JOIN ".$GLOBALS['xoopsDB']->prefix("users")." ON torrents.owner = users.id WHERE torrents.id = $id")
	or sqlerr();
