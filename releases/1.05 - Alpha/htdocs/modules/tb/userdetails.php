...
$GLOBALS['lang'] = array_merge( $GLOBALS['lang'], load_language('userdetails') );
...
function maketable($res)
    ...
    $r = $GLOBALS['xoopsDB']->queryF("SELECT t.id, t.name, t.seeders, t.leechers, c.name AS cname, c.image FROM ".$GLOBALS['xoopsDB']->prefix("tb_torrents")." t LEFT JOIN ".$GLOBALS['xoopsDB']->prefix("tb_categories")." c ON t.category = c.id WHERE t.owner = $id ORDER BY t.name") or sqlerr(__FILE__,__LINE__);
    if (mysql_num_rows($r) > 0)
    {
      ...
      {
        //$r2 = $GLOBALS['xoopsDB']->queryF("SELECT name, image FROM ".$GLOBALS['xoopsDB']->prefix("tb_categories")." WHERE id=$a[category]") or sqlerr(__FILE__, __LINE__);
    ...
    $country = '';
    $res = $GLOBALS['xoopsDB']->queryF("SELECT name,flagpic FROM ".$GLOBALS['xoopsDB']->prefix("tb_countries")." WHERE id=".$user['country']." LIMIT 1") or sqlerr();
    ...
    $res = $GLOBALS['xoopsDB']->queryF("SELECT p.torrent, p.uploaded, p.downloaded, p.seeder, t.added, t.name as torrentname, t.size, t.category, t.seeders, t.leechers, c.name as catname, c.image FROM ".$GLOBALS['xoopsDB']->prefix("tb_peers")." p LEFT JOIN ".$GLOBALS['xoopsDB']->prefix("tb_torrents")." t ON p.torrent = t.id LEFT JOIN ".$GLOBALS['xoopsDB']->prefix("tb_categories")." c ON t.category = c.id WHERE p.userid=$id") or sqlerr();
    ...
    {
      $r = $GLOBALS['xoopsDB']->queryF("SELECT id FROM ".$GLOBALS['xoopsDB']->prefix("tb_friends")." WHERE userid=".$GLOBALS['CURUSER']['id']." AND friendid=$id") or sqlerr(__FILE__, __LINE__);
    ...
    if ($GLOBALS['CURUSER']["id"] != $user["id"])
      ...
      {
        $r = $GLOBALS['xoopsDB']->queryF("SELECT id FROM ".$GLOBALS['xoopsDB']->prefix("tb_friends")." WHERE userid=$user[id] AND friendid=".$GLOBALS['CURUSER']['id']) or sqlerr(__FILE__,__LINE__);
    ...
    $xoopsOption['template_main'] = 'tb_userdetails.html';
	stdhead($title, '', '', 0);
	$GLOBALS['xoopsTpl']->assign('xoConfig', $GLOBALS['TBDEV']);
	$GLOBALS['xoopsTpl']->assign('html', $HTMLOUT);
	$GLOBALS['xoopsTpl']->assign('user', $user);
	$GLOBALS['xoopsTpl']->assign('xoops_pagetitle', $GLOBALS['lang']['userdetails_details']. " ". $user["username"]);
	stdfoot();
