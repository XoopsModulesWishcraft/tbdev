<?php

$GLOBALS['lang'] = array(

#Takerate
'rate_fail' => "Rating failed!",
'rate_login' => "Must be logged in to vote.",
'rate_miss_form_data' => "missing form data",
'rate_invalid_id' => "invalid id",
'rate_invalid' => "invalid rating",
'rate_torrent_not_found' => "no such torrent",
'rate_not_vote_own_torrent' => "You can't vote on your own ".$GLOBALS['xoopsDB']->prefix("tb_torrents").".",
'rate_already_voted' => "You have already rated this torrent.",


);

?>