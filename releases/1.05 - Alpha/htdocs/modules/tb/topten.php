...
require_once 'header.php';
  ...
  $GLOBALS['lang'] = array_merge( $GLOBALS['lang'], load_language('topten') );
  ...
  function peerstable($res, $frame_caption)
  {
    ...
    //  donortable($r, "Top 10 Donors");
      ...
      elseif ($type == 2)
      {
        ...
        {
          $r = $GLOBALS['xoopsDB']->queryF("SELECT t.*, (t.size * t.times_completed + SUM(p.downloaded)) AS data FROM ".$GLOBALS['xoopsDB']->prefix("tb_torrents")." AS t LEFT JOIN ".$GLOBALS['xoopsDB']->prefix("tb_peers")." AS p ON t.id = p.torrent WHERE p.seeder = 'no' GROUP BY t.id ORDER BY seeders + leechers DESC, seeders DESC, added ASC LIMIT $limit") or sqlerr();
        ...
        {
          $r = $GLOBALS['xoopsDB']->queryF("SELECT t.*, (t.size * t.times_completed + SUM(p.downloaded)) AS data FROM ".$GLOBALS['xoopsDB']->prefix("tb_torrents")." AS t LEFT JOIN ".$GLOBALS['xoopsDB']->prefix("tb_peers")." AS p ON t.id = p.torrent WHERE p.seeder = 'no' GROUP BY t.id ORDER BY times_completed DESC LIMIT $limit") or sqlerr();
      ...
      elseif ($type == 3)
      {
        ...
        {
          $r = $GLOBALS['xoopsDB']->queryF("SELECT name, flagpic, COUNT(".$GLOBALS['xoopsDB']->prefix("tb_users").".country) as num FROM ".$GLOBALS['xoopsDB']->prefix("tb_countries")." LEFT JOIN ".$GLOBALS['xoopsDB']->prefix("tb_users")." ON ".$GLOBALS['xoopsDB']->prefix("tb_users").".country = countries.id GROUP BY name ORDER BY num DESC LIMIT $limit") or sqlerr();
        ...
        if ($limit == 10 || $subtype == "ul")
        {
          $r = $GLOBALS['xoopsDB']->queryF("SELECT c.name, c.flagpic, sum(u.uploaded) AS ul FROM ".$GLOBALS['xoopsDB']->prefix("tb_users")." AS u LEFT JOIN ".$GLOBALS['xoopsDB']->prefix("tb_countries")." AS c ON u.country = c.id WHERE u.enabled = 'yes' GROUP BY c.name ORDER BY ul DESC LIMIT $limit") or sqlerr();
    ...
    //		print("<h1 align='center'><font color=''red''>Under construction!</font></h1>\n");
        if (!$limit || $limit > 250)
          ...
          if ($limit == 10 || $subtype == "ul")
          {
    //				$r = $GLOBALS['xoopsDB']->queryF("SELECT ".$GLOBALS['xoopsDB']->prefix("tb_users").".id AS userid, ".$GLOBALS['xoopsDB']->prefix("tb_peers").".id AS peerid, username, ".$GLOBALS['xoopsDB']->prefix("tb_peers").".uploaded, ".$GLOBALS['xoopsDB']->prefix("tb_peers").".downloaded, ".$GLOBALS['xoopsDB']->prefix("tb_peers").".uploaded / (UNIX_TIMESTAMP(NOW()) - (UNIX_TIMESTAMP(NOW()) - UNIX_TIMESTAMP(last_action)) - UNIX_TIMESTAMP(started)) AS uprate, ".$GLOBALS['xoopsDB']->prefix("tb_peers").".downloaded / (UNIX_TIMESTAMP(NOW()) - (UNIX_TIMESTAMP(NOW()) - UNIX_TIMESTAMP(last_action)) - UNIX_TIMESTAMP(started)) AS downrate FROM ".$GLOBALS['xoopsDB']->prefix("tb_peers")." LEFT JOIN ".$GLOBALS['xoopsDB']->prefix("tb_users")." ON ".$GLOBALS['xoopsDB']->prefix("tb_peers").".userid = ".$GLOBALS['xoopsDB']->prefix("tb_users").".id ORDER BY uprate DESC LIMIT $limit") or sqlerr();
    ...
    //				$r = $GLOBALS['xoopsDB']->queryF("SELECT ".$GLOBALS['xoopsDB']->prefix("tb_users").".id AS userid, ".$GLOBALS['xoopsDB']->prefix("tb_peers").".id AS peerid, username, ".$GLOBALS['xoopsDB']->prefix("tb_peers").".uploaded, ".$GLOBALS['xoopsDB']->prefix("tb_peers").".downloaded, (".$GLOBALS['xoopsDB']->prefix("tb_peers").".uploaded - ".$GLOBALS['xoopsDB']->prefix("tb_peers").".uploadoffset) / (UNIX_TIMESTAMP(last_action) - UNIX_TIMESTAMP(started)) AS uprate, (".$GLOBALS['xoopsDB']->prefix("tb_peers").".downloaded - ".$GLOBALS['xoopsDB']->prefix("tb_peers").".downloadoffset) / (UNIX_TIMESTAMP(last_action) - UNIX_TIMESTAMP(started)) AS downrate FROM ".$GLOBALS['xoopsDB']->prefix("tb_peers")." LEFT JOIN ".$GLOBALS['xoopsDB']->prefix("tb_users")." ON ".$GLOBALS['xoopsDB']->prefix("tb_peers").".userid = ".$GLOBALS['xoopsDB']->prefix("tb_users").".id ORDER BY uprate DESC LIMIT $limit") or sqlerr();
    //				peerstable($r, "Top $limit Fastest Uploaders (timeout corrected)" . ($limit == 10 && $pu ? " <font class='small'> - [<a href='topten.php?type=4&amp;lim=100&amp;subtype=ul'>Top 100</a>] - [<a href='topten.php?type=4&amp;lim=250&amp;subtype=ul'>Top 250</a>]</font>" : ""));
            ...
            $r = $GLOBALS['xoopsDB']->queryF( "SELECT ".$GLOBALS['xoopsDB']->prefix("tb_users").".id AS userid, username, (".$GLOBALS['xoopsDB']->prefix("tb_peers").".uploaded - ".$GLOBALS['xoopsDB']->prefix("tb_peers").".uploadoffset) / (last_action - started) AS uprate, IF(seeder = 'yes',(".$GLOBALS['xoopsDB']->prefix("tb_peers").".downloaded - ".$GLOBALS['xoopsDB']->prefix("tb_peers").".downloadoffset)  / (finishedat - started),(".$GLOBALS['xoopsDB']->prefix("tb_peers").".downloaded - ".$GLOBALS['xoopsDB']->prefix("tb_peers").".downloadoffset) / (last_action - started)) AS downrate FROM ".$GLOBALS['xoopsDB']->prefix("tb_peers")." LEFT JOIN ".$GLOBALS['xoopsDB']->prefix("tb_users")." ON ".$GLOBALS['xoopsDB']->prefix("tb_peers").".userid = ".$GLOBALS['xoopsDB']->prefix("tb_users").".id ORDER BY uprate DESC LIMIT $limit") or sqlerr();
          ...
          if ($limit == 10 || $subtype == "dl")
          {
    //				$r = $GLOBALS['xoopsDB']->queryF("SELECT ".$GLOBALS['xoopsDB']->prefix("tb_users").".id AS userid, ".$GLOBALS['xoopsDB']->prefix("tb_peers").".id AS peerid, username, ".$GLOBALS['xoopsDB']->prefix("tb_peers").".uploaded, ".$GLOBALS['xoopsDB']->prefix("tb_peers").".downloaded, (".$GLOBALS['xoopsDB']->prefix("tb_peers").".uploaded - ".$GLOBALS['xoopsDB']->prefix("tb_peers").".uploadoffset) / (UNIX_TIMESTAMP(last_action) - UNIX_TIMESTAMP(started)) AS uprate, (".$GLOBALS['xoopsDB']->prefix("tb_peers").".downloaded - ".$GLOBALS['xoopsDB']->prefix("tb_peers").".downloadoffset) / (UNIX_TIMESTAMP(last_action) - UNIX_TIMESTAMP(started)) AS downrate FROM ".$GLOBALS['xoopsDB']->prefix("tb_peers")." LEFT JOIN ".$GLOBALS['xoopsDB']->prefix("tb_users")." ON ".$GLOBALS['xoopsDB']->prefix("tb_peers").".userid = ".$GLOBALS['xoopsDB']->prefix("tb_users").".id ORDER BY downrate DESC LIMIT $limit") or sqlerr();
