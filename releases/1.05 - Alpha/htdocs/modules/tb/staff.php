...
loggedinorreturn();
    ...
    $GLOBALS['lang'] = array_merge( $GLOBALS['lang'], load_language('staff') );
    
    $HTMLOUT = '';
    
    $query = $GLOBALS['xoopsDB']->queryF("SELECT ".$GLOBALS['xoopsDB']->prefix("tb_users").".id, username, email, last_access, class, title, country, status, countries.flagpic, countries.name FROM ".$GLOBALS['xoopsDB']->prefix("tb_users")." LEFT  JOIN ".$GLOBALS['xoopsDB']->prefix("tb_countries")." ON countries.id = ".$GLOBALS['xoopsDB']->prefix("tb_users").".country WHERE class >=4 AND status='confirmed' ORDER BY username") or sqlerr();
    ...
    */
    function DoStaff($staff, $staffclass, $cols = 2) 
    {
      ...
      for($ia = 0; $ia < $rows; $ia++)
      {
          ...
          "   <img style='vertical-align: middle;' src='{$GLOBALS['TBDEV']['pic_base_url']}staff/".$GLOBALS['xoopsDB']->prefix("tb_users").".png' border='0' title=\"{$GLOBALS['lang']['alt_pm']}\" alt='' /></a>".
