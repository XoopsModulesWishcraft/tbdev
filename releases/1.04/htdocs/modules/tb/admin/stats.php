...
require "include/html_functions.php";
    ...
    $res = $GLOBALS['xoopsDB']->queryF("SELECT COUNT(*) FROM ".$GLOBALS['xoopsDB']->prefix("tb_torrents")) or sqlerr(__FILE__, __LINE__);
    $n = mysql_fetch_row($res);
    $n_tor = $n[0];

    $res = $GLOBALS['xoopsDB']->queryF("SELECT COUNT(*) FROM ".$GLOBALS['xoopsDB']->prefix("tb_peers")) or sqlerr(__FILE__, __LINE__);
    $n = mysql_fetch_row($res);
    $n_peers = $n[0];
    ...
    $query = "SELECT u.id, u.username AS name, MAX(t.added) AS last, COUNT(DISTINCT t.id) AS n_t, COUNT(p.id) as n_p
      FROM ".$GLOBALS['xoopsDB']->prefix("tb_users")." as u LEFT JOIN ".$GLOBALS['xoopsDB']->prefix("tb_torrents")." as t ON u.id = t.owner LEFT JOIN ".$GLOBALS['xoopsDB']->prefix("tb_peers")." as p ON t.id = p.torrent WHERE u.class = ". UC_UPLOADER ."
    ...
    else
    {
      ...
      FROM ".$GLOBALS['xoopsDB']->prefix("tb_categories")." as c LEFT JOIN ".$GLOBALS['xoopsDB']->prefix("tb_torrents")." as t ON t.category = c.id LEFT JOIN ".$GLOBALS['xoopsDB']->prefix("tb_peers")." as p
    ...
    $GLOBALS['tbTpl']->display('db:tb_cpanel_stats.html');
	xoops_cp_footer();
	exit(0);
