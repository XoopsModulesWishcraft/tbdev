...
|   $Revision: 208 $
|   $Author: tbdevnet $
|   $URL: admin/".$GLOBALS['xoopsDB']->prefix("tb_categories").".php $
+------------------------------------------------
*/
...
function move_cat() {
    ...
    // make sure both ".$GLOBALS['xoopsDB']->prefix("tb_categories")." exist
    $q = @$GLOBALS['xoopsDB']->queryF( "SELECT id FROM ".$GLOBALS['xoopsDB']->prefix("tb_categories")." WHERE id IN($old_cat_id, $new_cat_id)" );
    ...
    //all go
    @$GLOBALS['xoopsDB']->queryF( "UPDATE ".$GLOBALS['xoopsDB']->prefix("tb_torrents")." SET category = $new_cat_id WHERE category = $old_cat_id" );
...
function move_cat_form() {
    ...
    $q = @$GLOBALS['xoopsDB']->queryF( "SELECT * FROM ".$GLOBALS['xoopsDB']->prefix("tb_categories")." WHERE id = ".intval($params['id']) );
    ...
    $htmlout .= "<form action='admin.php?action=categories' method='post'>
      ...
      <tr>
        <td colspan='2'>Note: This tool will move ALL ".$GLOBALS['xoopsDB']->prefix("tb_torrents")." FROM one category to ANOTHER category only! It will NOT delete any ".$GLOBALS['xoopsDB']->prefix("tb_categories")." or ".$GLOBALS['xoopsDB']->prefix("tb_torrents").".</td>
      </tr>
      <tr>
...
function add_cat() {
    ...
    @$GLOBALS['xoopsDB']->queryF( "INSERT INTO ".$GLOBALS['xoopsDB']->prefix("tb_categories")." (name, cat_desc, image)
...
function delete_cat() {
    ...
    if( isset($params['new_cat_id']) )
    {
      ...
      $q = @$GLOBALS['xoopsDB']->queryF( "SELECT COUNT(*) FROM ".$GLOBALS['xoopsDB']->prefix("tb_categories")." WHERE id = $new_cat_id" );
    ...
    @$GLOBALS['xoopsDB']->queryF( "DELETE FROM ".$GLOBALS['xoopsDB']->prefix("tb_categories")." WHERE id = $old_cat_id" );
...
function delete_cat_form() {
    ...
    $q = @$GLOBALS['xoopsDB']->queryF( "SELECT COUNT(*) FROM ".$GLOBALS['xoopsDB']->prefix("tb_torrents")." WHERE category = ".intval($r['id']) );
    ...
    if($count[0])
    {
      ...
      $check .= "<tr>
        <td align='right' width='50%'>Select a new category:<br /><span style='color:red;font-weight:bold;'>Warning: There are ".$GLOBALS['xoopsDB']->prefix("tb_torrents")." in this category, so you need to select a category to move them to.</span></td>
        <td>$select</td>
      </tr>";
    }
...
function edit_cat() {
    ...
    @$GLOBALS['xoopsDB']->queryF( "UPDATE ".$GLOBALS['xoopsDB']->prefix("tb_categories")." SET name = $cat_name, cat_desc = $cat_desc, image = $cat_image WHERE id = $cat_id" );
...
function edit_cat_form() {
    ...
    $r = $GLOBALS['xoopsDB']->fetchArray($q);
    if (is_dir($GLOBALS['TBDEV']['pic_base_path'].'caticons')) {
	    $dh = opendir( $GLOBALS['TBDEV']['pic_base_path'].'caticons' );
...
function show_categories() {
    ...
    {
      $htmlout .= "<tr><td colspan='7'><h1>No ".$GLOBALS['xoopsDB']->prefix("tb_categories")." defined.</h1></td></tr>";
    } 
    else 
    {
      while($row = $GLOBALS['xoopsDB']->fetchArray($query))
      {
        $cat_image = file_exists($GLOBALS['TBDEV']['pic_base_path'].'caticons/'.$row['image']) ? "<img border='0' src='{$GLOBALS['TBDEV']['pic_base_url']}caticons/{$row['image']}' alt='{$row['id']}' />" : "No Image";
