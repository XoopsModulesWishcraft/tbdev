<?php

$GLOBALS['lang'] = array(

#upload errors
'upload_sorry' => "Sorry...",
'upload_no_auth' => "You are not authorized to upload ".$GLOBALS['xoopsDB']->prefix("tb_torrents").".  (See <a href='faq.php#up'>Uploading</a> in the FAQ.)",
'upload_announce_url' => "The tracker's announce url is",
'upload_delete' => "Delete",

#upload options
'upload_torrent' => "Torrent file",
'upload_name' => "Torrent name",
'upload_filename' => "Taken from filename if not specified. <b>Please use descriptive names.</b>",
'upload_nfo' => "NFO file",
'upload_nfo_info' => "<b>Optional.</b> Can only be viewed by power ".$GLOBALS['xoopsDB']->prefix("tb_users").".",
'upload_description' => "Description",
'upload_html_bbcode' => "HTML/BB code is <b>not</b> allowed.",
'upload_choose_one' => "choose one",
'upload_type' => "Type",
'upload_submit' => "Do it!",

#upload stdhead
'upload_stdhead' => "Upload"

);

?>