...
function genrelist() {
    $ret = array();
    $res = $GLOBALS['xoopsDB']->queryF("SELECT id, name FROM ".$GLOBALS['xoopsDB']->prefix("tb_categories")." ORDER BY name");
...
function write_log($text)
{
  $text = sqlesc($text);
  $added = TIME_NOW;
  $GLOBALS['xoopsDB']->queryF("INSERT INTO ".$GLOBALS['xoopsDB']->prefix("tb_sitelog")." (added, txt) VALUES($added, $text)") or sqlerr(__FILE__, __LINE__);
}
...
function StatusBar() {
	...
	$res2 = @$GLOBALS['xoopsDB']->queryF("SELECT seeder, COUNT(*) AS pCount FROM ".$GLOBALS['xoopsDB']->prefix("tb_peers")." WHERE userid=".$GLOBALS['CURUSER']['id']." GROUP BY seeder") or sqlerr(__LINE__,__FILE__);
