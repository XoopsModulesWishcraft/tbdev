...
/////////////// REP SYSTEM /////////////
//$GLOBALS['CURUSER']['reputation'] = 650;
	...
	if( $rep_is_on )
		{
			...
			//$rep_is_on = TRUE;
			//$GLOBALS['CURUSER']['g_rep_hide'] = FALSE;
			...
			if( $mode === 0 )
			return "Rep: ".$posneg . "<br /><a href='javascript:;' onclick=\"PopUp('{$GLOBALS['TBDEV']['baseurl']}/reputation.php?pid={$user['id']}','Reputation',400,241,1,1);\"><img src='./pic/plus.gif' border='0' alt='Add reputation:: {$user['username']}' title='Add reputation:: {$user['username']}' /></a>";
			else
			return "Rep: ".$posneg;
...
function get_user_icons($arr, $big = false)
  {
    ...
    $pics = $arr["donor"] == "yes" ? "<img src=\"{$GLOBALS['TBDEV']['pic_base_url']}{$donorpic}\" alt='Donor' border='0' $style />" : "";
    if ($arr["enabled"] == "yes")
      $pics .= $arr["warned"] == "yes" ? "<img src=\"{$GLOBALS['TBDEV']['pic_base_url']}{$warnedpic}\" alt=\"Warned\" border='0' $style />" : "";
    else
      $pics .= "<img src=\"{$GLOBALS['TBDEV']['pic_base_url']}{$disabledpic}\" alt=\"Disabled\" border='0' $style />\n";
    return $pics;
}
