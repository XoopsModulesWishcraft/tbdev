...
*/
    ...
    $torrents = number_format(get_row_count("".$GLOBALS['xoopsDB']->prefix("tb_torrents").""));
    //$dead = number_format(get_row_count("torrents", "WHERE visible='no'"));

    $r = $GLOBALS['xoopsDB']->queryF("SELECT value_u FROM ".$GLOBALS['xoopsDB']->prefix("tb_avps")." WHERE arg='seeders'") or sqlerr(__FILE__, __LINE__);
    $a = mysql_fetch_row($r);
    $seeders = 0 + $a[0];
    $r = $GLOBALS['xoopsDB']->queryF("SELECT value_u FROM ".$GLOBALS['xoopsDB']->prefix("tb_avps")." WHERE arg='leechers'") or sqlerr(__FILE__, __LINE__);
    $a = mysql_fetch_row($r);
    $leechers = 0 + $a[0];
    if ($leechers == 0)
      $ratio = 0;
    ...
    $res = $GLOBALS['xoopsDB']->queryF("SELECT * FROM ".$GLOBALS['xoopsDB']->prefix("tb_news")." WHERE added + ( 3600 *24 *45 ) >
