...
$res = $GLOBALS['xoopsDB']->queryF("SELECT $fields FROM ".$GLOBALS['xoopsDB']->prefix("tb_peers")." WHERE torrent = $torrentid AND connectable = 'yes' $limit");
...
if (!isset($self))
{
	$res = $GLOBALS['xoopsDB']->queryF("SELECT $fields FROM ".$GLOBALS['xoopsDB']->prefix("tb_peers")." WHERE $selfwhere");
...
$valid = @mysql_fetch_row(@$GLOBALS['xoopsDB']->queryF("SELECT COUNT(*) FROM ".$GLOBALS['xoopsDB']->prefix("tb_peers")." WHERE torrent=$torrentid AND passkey=" . sqlesc($passkey)));
...
if ($event == "stopped")
{
	if (isset($self))
	{
		$GLOBALS['xoopsDB']->queryF("DELETE FROM ".$GLOBALS['xoopsDB']->prefix("tb_peers")." WHERE $selfwhere");
...
else
{
	...
	if (isset($self))
	{
		$GLOBALS['xoopsDB']->queryF("UPDATE ".$GLOBALS['xoopsDB']->prefix("tb_peers")." SET uploaded = $uploaded, downloaded = $downloaded, to_go = $left, last_action = ".time().", seeder = '$seeder'"
	...
	else
	{
      ...
      $connectable = 'yes';
		...
		$ret = $GLOBALS['xoopsDB']->queryF("INSERT INTO ".$GLOBALS['xoopsDB']->prefix("tb_peers")." (connectable, torrent, peer_id, ip, port, uploaded, downloaded, to_go, started, last_action, seeder, userid, agent, passkey) VALUES ('$connectable', $torrentid, " . sqlesc($peer_id) . ", " . sqlesc($ip) . ", $port, $uploaded, $downloaded, $left, ".time().", ".time().", '$seeder', {$user['id']}, " . sqlesc($agent) . "," . sqlesc($passkey) . ")");
...
if (count($updateset))
	$GLOBALS['xoopsDB']->queryF("UPDATE ".$GLOBALS['xoopsDB']->prefix("tb_torrents")." SET " . join(",", $updateset) . " WHERE id = $torrentid");
