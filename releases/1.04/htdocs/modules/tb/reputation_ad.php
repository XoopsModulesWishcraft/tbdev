...
function show_level()
	{
		...
		$html = "<p>On this page you can modify the minimum amount required for each ".$GLOBALS['xoopsDB']->prefix("tb_reputation")." level. Make sure you press Update Minimum Levels to save your changes. You cannot set the same minimum amount to more than one level.<br />From here you can also choose to edit or remove any single level. Click the Edit link to modify the Level description (see Editing a Reputation Level) or click Remove to delete a level. If you remove a level or modify the minimum ".$GLOBALS['xoopsDB']->prefix("tb_reputation")." needed to be at a level, all ".$GLOBALS['xoopsDB']->prefix("tb_users")." will be updated to reflect their new level if necessary.</p><br />";

		$query = $GLOBALS['xoopsDB']->queryF( 'SELECT * FROM  '.$GLOBALS['xoopsDB']->prefix("tb_reputationlevel").'  ORDER BY minimumreputation ASC' );
		...
		while( $res = $GLOBALS['xoopsDB']->fetchArray( $query ) )
		{
			$html .= "<tr>\n".
					...
					"	<td align='center'><span class='btn'><a href='reputation_ad.php?mode=edit&amp; ".$GLOBALS['xoopsDB']->prefix("tb_reputationlevel").".id=".$res['reputationlevelid']."'>Edit</a></span>&nbsp;<span class='btn'><a href='reputation_ad.php?mode=dodelete&amp; ".$GLOBALS['xoopsDB']->prefix("tb_reputationlevel").".id=".$res['reputationlevelid']."'>Delete</a></span></td>\n".
					"</tr>\n";
		}
...
function show_form($type='edit')
	{
		...
		$html = "This allows you to add a new ".$GLOBALS['xoopsDB']->prefix("tb_reputation")." level or edit an existing ".$GLOBALS['xoopsDB']->prefix("tb_reputation")." level.";

		if( $type == 'edit' )
		{
			$query = $GLOBALS['xoopsDB']->queryF( 'SELECT * FROM  '.$GLOBALS['xoopsDB']->prefix("tb_reputationlevel").'  WHERE  '.$GLOBALS['xoopsDB']->prefix("tb_reputationlevel").'.id='.intval($input['reputationlevelid']) ) or sqlerr(__LINE__,__FILE__);
		...
		$html .= "<form action='reputation_ad.php' name='show_rep_form' method='post'>
				<input name='".$GLOBALS['xoopsDB']->prefix("tb_reputationlevel")."id' value='{$replevid}' type='hidden' />
		...
		$html .= "<tr><td>Level Description<div class='desctext'>This is what is displayed for the user when their ".$GLOBALS['xoopsDB']->prefix("tb_reputation")." points are above the amount entered as the minimum.</div></td>";
		$html .= "<td><input type='text' name='level' value=\"{$replevel}\" size='35' maxlength='250' /></td></tr>";
		$html .= "<tr><td>Minimum amount of ".$GLOBALS['xoopsDB']->prefix("tb_reputation")." points required for this level<div>This can be a positive or a negative amount. When the user's ".$GLOBALS['xoopsDB']->prefix("tb_reputation")." points reaches this amount, the above description will be displayed.</div></td>";
...
function do_update($type="")
	{
		...
		if( $type == 'new' )
		{
			@$GLOBALS['xoopsDB']->queryF( "INSERT INTO  ".$GLOBALS['xoopsDB']->prefix("tb_reputationlevel")."  ( minimumreputation, level ) 
							VALUES  ($minrep, $level )" );
		}
		elseif( $type == 'edit' )
		{
			...
			$query = $GLOBALS['xoopsDB']->queryF( "SELECT  ".$GLOBALS['xoopsDB']->prefix("tb_reputationlevel")." id FROM  ".$GLOBALS['xoopsDB']->prefix("tb_reputationlevel")."  WHERE 
									 ".$GLOBALS['xoopsDB']->prefix("tb_reputationlevel").".id={$levelid}" );
			...
			@$GLOBALS['xoopsDB']->queryF( "UPDATE  ".$GLOBALS['xoopsDB']->prefix("tb_reputationlevel")."  SET minimumreputation = $minrep, level = $level 
							WHERE  ".$GLOBALS['xoopsDB']->prefix("tb_reputationlevel")." id = $levelid" );
		}
		else
		{
			...
			if( is_array($ids) && count($ids) )
			{
				foreach( $ids as $k => $v )
				{
					@$GLOBALS['xoopsDB']->queryF( "UPDATE  ".$GLOBALS['xoopsDB']->prefix("tb_reputationlevel")."  SET minimumreputation = ".intval($v)." WHERE  ".$GLOBALS['xoopsDB']->prefix("tb_reputationlevel")." id = ".intval($k) );
				}
			}
			else
			{
...
function do_delete()
	{
		...
		$query = $GLOBALS['xoopsDB']->queryF( "SELECT  ".$GLOBALS['xoopsDB']->prefix("tb_reputationlevel")." id FROM  ".$GLOBALS['xoopsDB']->prefix("tb_reputationlevel")."  WHERE  ".$GLOBALS['xoopsDB']->prefix("tb_reputationlevel").".id=$levelid" );
		...
		@$GLOBALS['xoopsDB']->queryF( "DELETE FROM  ".$GLOBALS['xoopsDB']->prefix("tb_reputationlevel")."  WHERE  ".$GLOBALS['xoopsDB']->prefix("tb_reputationlevel").".id=$levelid" );
		rep_cache();
...
function show_form_rep()
	{
		...
		$query = $GLOBALS['xoopsDB']->queryF( "SELECT r.*, p.topicid, t.subject, leftfor.username as leftfor_name, 
					...
					FROM ".$GLOBALS['xoopsDB']->prefix("tb_reputation")." r
...
/////////////////////////////////////
//	View ".$GLOBALS['xoopsDB']->prefix("tb_reputation")." comments function
/////////////////////////////////////
...
function view_list()
	{
		...
		$html .= "<p>This page allows you to search for ".$GLOBALS['xoopsDB']->prefix("tb_reputation")." comments left by / for specific ".$GLOBALS['xoopsDB']->prefix("tb_users")." over the specified date range.</p>";
...
//print $html; exit;
		...
		if( isset($input['dolist']) )
		{
			...
			$query = $GLOBALS['xoopsDB']->queryF( "SELECT COUNT(*) AS cnt FROM ".$GLOBALS['xoopsDB']->prefix("tb_reputation")." r WHERE $cond" );
			//print_r($input); exit;
...
function do_delete_rep()
	{
		...
		$query = $GLOBALS['xoopsDB']->queryF( "SELECT reputationid, reputation, userid FROM ".$GLOBALS['xoopsDB']->prefix("tb_reputation")." WHERE reputationid=".intval($input['reputationid'] ) );
		...
		// do the delete
		@$GLOBALS['xoopsDB']->queryF( "DELETE FROM ".$GLOBALS['xoopsDB']->prefix("tb_reputation")." WHERE reputationid=".intval($r['reputationid'] ) );
		@$GLOBALS['xoopsDB']->queryF( "UPDATE ".$GLOBALS['xoopsDB']->prefix("tb_users")." SET ".$GLOBALS['xoopsDB']->prefix("tb_reputation")." = (reputation-{$r['reputation']} ) WHERE id=".intval($r['userid']) );
...
function do_edit_rep()
	{
		...
		// valid ID?
		$query = $GLOBALS['xoopsDB']->queryF( "SELECT reputationid, reason, userid FROM ".$GLOBALS['xoopsDB']->prefix("tb_reputation")." WHERE reputationid=".intval($input['reputationid'] ) );
...
/*
		if( $oldrep != $newrep )
		{
			if( $r['reason'] != $reason )
			{
				@$GLOBALS['xoopsDB']->queryF( "UPDATE ".$GLOBALS['xoopsDB']->prefix("tb_reputation")." SET ".$GLOBALS['xoopsDB']->prefix("tb_reputation")." = ".intval($newrep).", reason = ".sqlesc($reason).". WHERE reputationid = ".intval($r['reputationid']) );
			}

			$diff = $oldrep - $newrep;
			@$GLOBALS['xoopsDB']->queryF( "UPDATE ".$GLOBALS['xoopsDB']->prefix("tb_users")." SET ".$GLOBALS['xoopsDB']->prefix("tb_reputation")." = (reputation-{$diff}) WHERE id=".intval($r['userid']) );
...
function rep_cache()
	{
		...
		$query = @$GLOBALS['xoopsDB']->queryF( "SELECT * FROM  ".$GLOBALS['xoopsDB']->prefix("tb_reputationlevel")." " );
