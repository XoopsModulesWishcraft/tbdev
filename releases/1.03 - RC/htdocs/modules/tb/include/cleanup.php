...
function docleanup() {
	...
	do {
		...
		if (count($delids))
			$GLOBALS['xoopsDB']->queryF("DELETE FROM ".$GLOBALS['xoopsDB']->prefix("torrents")." WHERE id IN (" . join(",", $delids) . ")");

		$res = $GLOBALS['xoopsDB']->queryF("SELECT torrent FROM ".$GLOBALS['xoopsDB']->prefix("peers")." GROUP BY torrent");
		$delids = array();
		while ($row = mysql_fetch_array($res,MYSQL_NUM)) {
			$id = $row[0];
			if (isset($ar[$id]) && $ar[$id])
		...
		if (count($delids))
			$GLOBALS['xoopsDB']->queryF("DELETE FROM ".$GLOBALS['xoopsDB']->prefix("peers")." WHERE torrent IN (" . join(",", $delids) . ")");

		$res = $GLOBALS['xoopsDB']->queryF("SELECT torrent FROM ".$GLOBALS['xoopsDB']->prefix("files")." GROUP BY torrent");
		$delids = array();
		while ($row = mysql_fetch_array($res,MYSQL_NUM)) {
			$id = $row[0];
			if (isset($ar[$id]) && $ar[$id])
		...
		if (count($delids))
			$GLOBALS['xoopsDB']->queryF("DELETE FROM ".$GLOBALS['xoopsDB']->prefix("files")." WHERE torrent IN (" . join(",", $delids) . ")");
	} while (0);

	$deadtime = deadtime();
	@$GLOBALS['xoopsDB']->queryF("DELETE FROM ".$GLOBALS['xoopsDB']->prefix("peers")." WHERE last_action < $deadtime");
	...
	while ($row = $GLOBALS['xoopsDB']->fetchArray($res)) {
		...
		if (count($update))
			@$GLOBALS['xoopsDB']->queryF("UPDATE ".$GLOBALS['xoopsDB']->prefix("torrents")." SET " . implode(",", $update) . " WHERE id = $id");
	}
...
*/  
  ...
  if (mysql_num_rows($res) > 0)
  {
	...
	// Update stats
	$seeders = get_row_count($GLOBALS['xoopsDB']->prefix("peers"), "WHERE seeder='yes'");
	$leechers = get_row_count($GLOBALS['xoopsDB']->prefix("peers"), "WHERE seeder='no'");
	@$GLOBALS['xoopsDB']->queryF("UPDATE ".$GLOBALS['xoopsDB']->prefix("avps")." SET value_u=$seeders WHERE arg='seeders'") or sqlerr(__FILE__, __LINE__);
	@$GLOBALS['xoopsDB']->queryF("UPDATE ".$GLOBALS['xoopsDB']->prefix("avps")." SET value_u=$leechers WHERE arg='leechers'") or sqlerr(__FILE__, __LINE__);
	...
	$days = 28;
	$dt = (time() - ($days * 86400));
	$res = $GLOBALS['xoopsDB']->queryF("SELECT id, name FROM ".$GLOBALS['xoopsDB']->prefix("torrents")." WHERE added < $dt");
	while ($arr = $GLOBALS['xoopsDB']->fetchArray($res))
	{
		@unlink("{$GLOBALS['TBDEV']['torrent_dir']}/{$arr['id']}.torrent");
		@$GLOBALS['xoopsDB']->queryF("DELETE FROM ".$GLOBALS['xoopsDB']->prefix("torrents")." WHERE id={$arr['id']}");
		@$GLOBALS['xoopsDB']->queryF("DELETE FROM ".$GLOBALS['xoopsDB']->prefix("peers")." WHERE torrent={$arr['id']}");
		@$GLOBALS['xoopsDB']->queryF("DELETE FROM comments WHERE torrent={$arr['id']}");
		@$GLOBALS['xoopsDB']->queryF("DELETE FROM ".$GLOBALS['xoopsDB']->prefix("files")." WHERE torrent={$arr['id']}");
