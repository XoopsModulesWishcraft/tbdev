...
if (count($_POST) > 0 );//&& isset($_POST['n']))
{
  ...
  {
  	$distinct = "DISTINCT ";
    $join_is .= " LEFT JOIN ".$GLOBALS['xoopsDB']->prefix("peers")." AS p ON u.id = p.userid";
    $q .= ($q ? "&amp;" : "") . "ac=$active";
  }
...
//    </temporary>   /////////////////////////////////////////////////////
  ...
  else
  {
    ...
    while ($user = mysql_fetch_array($res))
    {
      ...
      {
	    	$nip = ip2long($user['ip']);
        $auxres = $GLOBALS['xoopsDB']->queryF("SELECT COUNT(*) FROM ".$GLOBALS['xoopsDB']->prefix("bans")." WHERE $nip >= first AND $nip <= last") or sqlerr(__FILE__, __LINE__);
      ...
      $auxres = $GLOBALS['xoopsDB']->queryF("SELECT SUM(uploaded) AS pul, SUM(downloaded) AS pdl FROM ".$GLOBALS['xoopsDB']->prefix("peers")." WHERE userid = ".$user['id']) or sqlerr(__FILE__, __LINE__);
...
//    $auxres = $GLOBALS['xoopsDB']->queryF("SELECT COUNT(id) FROM posts WHERE userid = ".$user['id']) or sqlerr(__FILE__, __LINE__);
      ...
      // $auxres = $GLOBALS['xoopsDB']->queryF("SELECT COUNT(c.id) FROM comments AS c LEFT JOIN ".$GLOBALS['xoopsDB']->prefix("torrents")." as t ON c.torrent = t.id WHERE c.user = '".$user['id']."'") or sqlerr(__FILE__, __LINE__);
      $n = mysql_fetch_row($auxres);
      $n_comments = $n[0];
      $ids .= $user['id'].':';
