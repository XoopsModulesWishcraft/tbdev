...
if (is_object($GLOBALS['xoopsUser'])) {
    ...
    if (!$row) {
	    ...
	    $ret = $GLOBALS['xoopsDB']->queryF("INSERT INTO ".$GLOBALS['xoopsDB']->prefix("tb_users")." (username, uid, passhash, secret, editsecret, email, status, class, added, time_offset, dst_in_use) VALUES (" .
			implode(",", array_map("sqlesc", array($GLOBALS['xoopsUser']->getVar('uname'), $GLOBALS['xoopsUser']->getVar('tbuid'), $GLOBALS['xoopsUser']->getVar('pass'), $secret, $editsecret, $GLOBALS['xoopsUser']->getVar('email'), 'confirmed'))).
