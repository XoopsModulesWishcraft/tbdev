...
loggedinorreturn();
    ...
    <td align='center' width='33%'><a href='my".$GLOBALS['xoopsDB']->prefix("torrents").".php'><b>{$lang['my_torrents']}</b></a></td>
    ...
    <tr>
      ...
      $user_header<br />
      $avatar<br />
      <a href='my".$GLOBALS['xoopsDB']->prefix("torrents").".php'>{$lang['my_edit_torrents']}</a><br />
      <a href='friends.php'>{$lang['my_edit_friends']}</a><br />
      <a href='".$GLOBALS['xoopsDB']->prefix("tb_users").".php'>{$lang['my_search']}</a>
      </td>
    <td>
    ...
    $res = $GLOBALS['xoopsDB']->queryF("SELECT COUNT(*) FROM ratings WHERE user=" . $CURUSER["id"]);
    $row = mysql_fetch_array($res,MYSQL_NUM);
    tr("Ratings submitted", $row[0]);

    $res = $GLOBALS['xoopsDB']->queryF("SELECT COUNT(*) FROM comments WHERE user=" . $CURUSER["id"]);
    $row = mysql_fetch_array($res,MYSQL_NUM);
    tr("Written comments", $row[0]);

    ****************/
    $stylesheets ='';
    $ss_r = $GLOBALS['xoopsDB']->queryF("SELECT * from ".$GLOBALS['xoopsDB']->prefix("stylesheets")."") or die;
    $ss_sa = array();
    while ($ss_a = $GLOBALS['xoopsDB']->fetchArray($ss_r))
    {
      $ss_id = $ss_a["id"];
      $ss_name = $ss_a["name"];
      $ss_sa[$ss_name] = $ss_id;
    ...
    $ct_r = $GLOBALS['xoopsDB']->queryF("SELECT id,name FROM ".$GLOBALS['xoopsDB']->prefix("countries")." ORDER BY name") or sqlerr(__FILE__,__LINE__);
    
    while ($ct_a = $GLOBALS['xoopsDB']->fetchArray($ct_r))
    {
    ...
    $r = $GLOBALS['xoopsDB']->queryF("SELECT id,name FROM ".$GLOBALS['xoopsDB']->prefix("categories")." ORDER BY name") or sqlerr();
    //$categories = "Default browsing categories:<br>\n";
    if ($GLOBALS['xoopsDB']->getRowsNum($r) > 0)
    {
      $categories .= "<table><tr>\n";
      $i = 0;
    ...
    */
    //print("<p><a href='".$GLOBALS['xoopsDB']->prefix("tb_users").".php'><b>Find User/Browse User List</b></a></p>");
